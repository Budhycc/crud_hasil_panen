...
if (isset($_GET['search'])) {
  $search = $_GET['search'];
  $sql = "SELECT * FROM desa WHERE nama_desa LIKE '%$search%' OR  kd_desa LIKE '%$search%'";
} else {
    ...
    <div class="nav-scroller py-1 mb-2">
      <nav class="nav d-flex justify-content-center">
        <div class="nav nav-tabs" id="nav-tab" role="tablist">
          ...
          <a class="nav-item nav-link active" id="nav-contact-tab" data-toggle="tab" href="data_desa.php" role="tab"
  ...
  <main class="container">
    <article class="blog-post">
      <div class="container-fluid mt-3">
        <h2 class="lead">Data Desa</h2>


        <?php if (isset($_SESSION['nik'])) { ?>
          <?php if ($_SESSION['level'] == "ADMIN") { ?>
            <a href="kelola_desa.php" type="button" class="btn btn-primary mb-3">Tambah</a>
          <?php } ?>
        <?php } ?>

        <div class="row mb-3">
          <div class="col-lg-6">
            <form method="get" action="data_desa.php">
              <div class="input-group">
                <input type="text" class="form-control" placeholder="Cari berdasarkan kode dan desa"
                  name="search">
                <button class="btn btn-outline-secondary" type="submit">Cari</button>
                <a href="data_desa.php" type="button" class="btn btn-outline-secondary">Batal</a>
              </div>
            </form>
          </div>
        </div>
            ...
            while ($row = mysqli_fetch_assoc($result)) {
              ...
              <tr>
                ...
                <td>
                  <?php if (isset($_SESSION['nik'])) { ?>
                    <?php if ($_SESSION['level'] == "ADMIN") { ?>
                      <a href="kelola_desa.php?edit=<?php echo $row['kd_desa']; ?>" type="button"
                        class="btn btn-success btn-sm">
                        Edit
                      </a>
                      <a href="proses_desa.php?hapus=<?php echo $row['kd_desa']; ?>" type="button"
